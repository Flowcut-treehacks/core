...
class PlanBuilder:
    ...
    def __init__(self):
        ...
        self._prompt = ""
        self._plan_db_id: Optional[int] = None

    def start_plan(self, user_prompt: str, plan_id: Optional[int] = None) -> None:
        """Start a new plan for this edit run. plan_id from storage.create_plan() for step persistence."""
        with self._lock:
            self._prompt = user_prompt or ""
            self._plan_db_id = plan_id
    ...
    def add_step(
        ...
        snapshot_before: serialized project state before the tool ran (for revert).
        """
        with self._lock:
            if self._root is None:
                return
            ...
            branch_key = parent.get("branch_key", "general") if parent else "general"
            if self._plan_db_id is not None:
                try:
                    from plan_graph.storage import add_step as persist_step
                    status = "fail" if (result or "").strip().startswith("Error") else "success"
                    persist_step(
                        step_id=node["id"],
                        plan_id=self._plan_db_id,
                        branch=branch_key,
                        tool_name=tool_name,
                        input_args=args_json or "{}",
                        output_result=result or "",
                        status=status,
                        label=label,
    ...
    def get_plan_id(self) -> Optional[int]:
        """Return the DB plan_id for the current plan, or None."""
        with self._lock:
            return self._plan_db_id

    def update_step_args(self, step_id: str, new_args_json: str) -> bool:
        """Update a step node's args (for edit-dialog save). Returns True if node found."""
        try:
            args = json.loads(new_args_json) if new_args_json else {}
        except Exception:
            return False
        with self._lock:
            node = self._nodes_by_id.get(step_id)
            if node is None or node.get("type") != "step":
                return False
            node["args"] = args if isinstance(args, dict) else {}
        return True
